...
export default function HeroAnimation({ text1, text2, speed = 100 }) {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (typeStatus === 'typing' && currentIndex < (isText1 ? text1.length : text2.length)) {
        // Typing forward
        setDisplayText((prev) => prev + (isText1 ? text1[currentIndex] : text2[currentIndex]))
  ...
  return (
    <p className="inline-flex text-inherit">
      <span className=" to-foreground from-muted-foreground bg-gradient-to-t to-70% bg-clip-text font-semibold text-transparent sm:bg-gradient-to-r">
        {displayText}
      </span>
      <span className={`${showCursor ? 'text-muted-foreground' : 'hidden'}`}>|</span>
    </p>
  )
}
