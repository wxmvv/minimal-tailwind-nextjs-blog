...
function shouldUseColorThief(song: AudioInfo | undefined, fallback = defaultThemeColor) {
  if (song?.theme === 'auto' && song.cover) return true
  return fallback === 'auto' && Boolean(song?.cover)
}

export function useThemeColor(song: AudioInfo | undefined, fallback = defaultThemeColor) {
