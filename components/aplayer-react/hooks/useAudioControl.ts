...
function useCreateAudioElement(options?: CreateAudioElementOptions) {
  ...
  useEffect(() => {
    ...
    if (audio && options?.onError) {
      audio.addEventListener('error', options.onError as unknown as (event: ErrorEvent) => void)

      return () => {
        audio.removeEventListener(
          'error',
          options.onError as unknown as (event: ErrorEvent) => void
        )
      }
    }
  }, [options?.onError])
  ...
  useEffect(() => {
    ...
    if (audio && options?.onEnded) {
      audio.addEventListener('ended', options.onEnded as unknown as (event: ErrorEvent) => void)

      return () => {
        audio.removeEventListener(
          'ended',
          options.onEnded as unknown as (event: ErrorEvent) => void
        )
      }
    }
  }, [options?.onEnded])
